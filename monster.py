...
TELEGRAM_BOT_TOKEN = '8052597093:AAEd_k1zZ2_6rrycoPpWf4YDEV8dnbYqnoU'
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users or get_remaining_approval_time(user_id) == "Expired":
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ **APNI USER ID ADD KARO** .*", parse_mode='Markdown')
        return

    if attack_in_progress:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ **PHALA SA LGA HUA HA WAIT KAR**.*", parse_mode='Markdown')
        return
